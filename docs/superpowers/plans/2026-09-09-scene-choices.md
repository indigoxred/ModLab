...
Subsequent integration is recorded in `docs/SCENE_CHOICES_CHECKPOINT_2026-09-09.md`: saved requests, packed dependencies, background preparation, native activation/reset coordination and the two-tab Graphics panel are now implemented in source. A private real-file build checks 277 outputs and 55 dependencies. The four acceptance items above remain unchecked until native panel/application/reopen/reset verification and downstream reporting are complete; Computer Use capture failed during the initial deployment check. Later source corrections still need deployment with MO2 closed.

Native follow-up: capture recovered and current `12e784e` modules were deployed with MO2 closed. The real panel selected Blended Roads for roads and Majestic Mountains for mountains independently, applied 277 checked files/55 dependencies, and restored the selections on reopening. Five installed bridge patch files were preserved. Downstream graphics preparation consumed all 277 scene entries and published 654 checked meshes/656 files. Renderer controls remain accessible; closing the panel did not start helpers. Remaining acceptance gaps are native reset/reapply and clear current-result reporting after downstream transformation. See the updated checkpoint for job identifiers and evidence boundaries.
