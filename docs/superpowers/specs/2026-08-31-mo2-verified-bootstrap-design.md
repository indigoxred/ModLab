...
The five fixed masters are `Skyrim.esm`, `Update.esm`, `Dawnguard.esm`, `HearthFires.esm`, and `Dragonborn.esm`. Each must exist as an observed top-level `Data` plug-in before Create is eligible. `Skyrim.ccc` is parsed with the same strict plug-in-name rules used by the current MO2 adapter, including Windows-invalid characters and reserved device names. It is a catalogue/order source and can legitimately list Creation content not installed by an SE owner, so absent entries are omitted; present entries must be direct regular top-level `Data` files. Duplicates, unsafe names, or present-but-unsafe entries block. Plug-in membership is observed and stability-checked through metadata without loading whole master payloads; this policy does not claim their content identity. `Skyrim.ccc` presence, bytes, size, SHA-256, and the resulting installed ordered plug-in sequence are frozen in the plan and rechecked at apply.
