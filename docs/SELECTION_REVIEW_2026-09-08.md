...
## Follow-up review of 417734c

The follow-up correctly identified another still-present defect: the newly named custom preset reset current body-part/outfit decisions. A signal-aware test exercising the actual customization callback and refresh method reproduced four failures: excluded outfit, unanswered variant, selected alternative and now-unavailable project were replaced by the suggestion. The callback now captures pending decisions before rebuilding controls, selects the published preset without the destructive intermediate signal, and carries decisions into the rebuilt output-path groups. Unavailable selected projects remain explicit review choices; the existing build planner rejects them. Pending persistence remains separate from applied defaults.

Validation: all 394 hub tests pass after correction; diff whitespace check passes. This return-path check uses lightweight controls with preset-change signals, not a fresh native MO2/BodySlide session. It does not claim a new mesh build, deployment, per-character assignment or completion of independent character customization. The review's two earlier defects and pending-choice persistence were already addressed after its 417734c snapshot, in d476ff1 and 3f1c5af.
