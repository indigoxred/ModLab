...
Fresh desktop verification on 58b22db: job `802d06141274` displayed the CBBE NeverNude body and underwear with textures loaded; its BodySlide log had no missing-resource/open-file errors. Closing without saving returned the unchanged Athletic selection and a no-saved-change message. The prior edited preset hash remained correct in managed output. No mesh build or OBody installation was performed. The preset remains available after reopening, but the draft selection itself is not yet persisted across dialog reopening; that follow-up and overly broad downstream stale checks remain outstanding.
