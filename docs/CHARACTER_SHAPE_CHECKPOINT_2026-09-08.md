...
## Combined preparation applied in native ModLab — 9 September

The combined guided operation now runs the body build and character assignment publication together. The optional individual-shapes choice retains the user's desired shared preset while using the verified author's neutral build preset. Static preparation remains available without OBody. Character-level preparation requirements link directly into the guided body screen.

Native BodySlide job `acd08d628601` prepared 428 projects and applied 1,281 checked NIF/TRI files. Assignment job `14507b21911c` applied 30 female race defaults, including applicable races with no current NPCs, and Lydia's separate saved preset (`0A2C8E:skyrim.esm`). The shared preset is `ModLab - Shared female - CBBE NeverNude [efe413484e0a]`; Lydia uses `ModLab - Lydia - 0A2C8E-skyrim.esm [ea347aee6ca9]`. The build itself used the authenticated author preset `- Zeroed Sliders -`. Separate static NPC bodies and skins were retained. Source configuration was not overwritten.

The native dialog reported both publications effective. A separate verification rerun after automatic follow-up checked 1,293 recorded inputs and two archived inputs, the assignment rules, output manifests and absence of the incomplete-operation marker. Report: task workspace `outputs/ui-rework/verify_combined_shapes.json`. This is file/configuration evidence, not a gameplay verification. Existing saves may need the helper's assignment reset.

Interrupted/failed combined preparation retains a launch blocker and its preflight record. Recovery restores only the outputs published by that attempt, preserving manual edits and newer publications. Tests exercise a failed assignment after both outputs were published. Shared defaults are saved only after assignments succeed. The post-apply note is refreshed, and actual follow-up preparation navigates to Continue setup so progress is visible.

Validation: 443 hub tests pass and hub modules compile. Automatic Pandora, NPC, Synthesis, graphics and record checks finished after applying this setup. The running interface still reports unrelated unfinished installation/record findings; this is not a declaration that the whole profile or broader product is complete. Independent skin/hair/physics choices and the remaining guided workflows still require work.
