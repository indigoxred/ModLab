...
from .installation import inspect_install_result
...
def install_archive(organizer, archive: Path, on_ready=None):
    ...
    record = {
        ...
        "archive": str(archive), "profile": profile, "mods_root": str(mods_root),
    ...
    try:
        ...
        if organizer.profile().name() != profile or Path(organizer.modsPath()).resolve() != mods_root:
        ...
        if result.path and result.file_count:
            ...
            result = replace(result, status='Installed; activation pending',
                             detail='Files are installed. Waiting for MO2 to refresh before enabling them.')
            ...
            organizer.onNextRefresh(lambda: QTimer.singleShot(0, activate))
